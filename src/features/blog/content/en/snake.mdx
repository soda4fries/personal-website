...
The implementation uses React with a requestAnimationFrame game loop for smooth gameplay. I added an A* pathfinding algorithm that calculates the optimal route in real-time, plus an input queue system that buffers rapid key presses and applies them over two animation ticks (learned this the hard way because if you're going one way then turn the opposite direction, you'd crash into yourself). 

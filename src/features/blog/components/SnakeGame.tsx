...
interface ActionHistoryItem {
  type: 'applied_immediate' | 'applied_queue' | 'queued' | 'cancelled' | 'game_over';
  direction: Direction;
...
const SnakeGameWithQueue = () => {
  ...
  const gameStateRef = useRef<GameState>({ gameStarted: false, gameOver: false });
    ...
    while (openSet.length > 0) {
      ...
      for (const neighbor of neighbors) {
        ...
        if (neighbor.x < 0 || neighbor.x >= GRID_SIZE || 
            ...
            snakeBody.some(segment => segment.x === neighbor.x && segment.y === neighbor.y) ||
  ...
  const changeDirection = useCallback((newDirection: Direction) => {
    ...
    setDirectionQueue(currentQueue => {
      ...
      const lastDirection = currentQueue.length > 0 
      ...
      if (isOpposite || 
          (lastDirection.x === newDirection.x && lastDirection.y === newDirection.y)) {
    ...
    if (loggedAction) {
      setActionHistory(prev => [...prev.slice(-19), loggedAction as ActionHistoryItem]);
    }
  }, []);
  ...
  const gameLoop = useCallback((currentTime: number) => {
    ...
    if (currentTime - lastMoveTimeRef.current >= MOVE_INTERVAL) {
      ...
      if (directionQueueRef.current.length > 0) {
        ...
        setActionHistory(prev => [...prev.slice(-19), {
          type: 'applied_queue',
      ...
      if (head.x === food.x && head.y === food.y) {
        ...
        setMoveCount(currentMoves => {
          if (storedOptimalMoves > 0 && currentMoves > 0) {
            console.log(`Efficiency calc: ${storedOptimalMoves} optimal / ${currentMoves} actual = ${(storedOptimalMoves / currentMoves) * 100}%`);
            const efficiency = Math.round((storedOptimalMoves / currentMoves) * 100);
            ...
            setTotalFoodCollected(prevTotal => {
              ...
              setAverageEfficiency(prevAvg => {
                const newAverage = Math.round(((prevAvg * prevTotal) + efficiency) / newTotal);
                return newAverage;
              });
              return newTotal;
            });
    ...
    <div className="flex flex-col items-center gap-8 my-6">
      <div className="flex flex-col items-center gap-8">
        ...
        <div className="flex flex-col items-center">
          ...
          >
            {Array.from({ length: GRID_SIZE * GRID_SIZE }).map((_, index) => {
              ...
              const isPath = showTrainingPath && shortestPath.some(
                (segment) => segment.x === x && segment.y === y
              ) && !isSnake && !isFood;
                ...
                <div
                  ...
                  style={{
                    ...
                    backgroundColor: isFood
                      ...
                      ? "rgba(255, 111, 0, 0.2)"
                      : "transparent",
                    border: isPath ? "1px solid rgba(255, 111, 0, 0.4)" : "none",
                  }}
                />
              );
            })}
        ...
        <div className="flex flex-col md:flex-row gap-4 w-full max-w-2xl">
            ...
            <div className="space-y-2">
              ...
              <div className="border-t pt-2">
                <div className="flex justify-between text-sm mb-1">
                  ...
                  <span className={`font-bold ${currentEfficiency >= 80 ? 'text-green-600' : currentEfficiency >= 60 ? 'text-yellow-600' : 'text-red-600'}`}>
                    {currentEfficiency}%
                  </span>
                </div>
                <div className="flex justify-between text-sm mb-1">
                  <span className="text-gray-600">Average Efficiency:</span>
                  <span className={`font-bold ${averageEfficiency >= 80 ? 'text-green-600' : averageEfficiency >= 60 ? 'text-yellow-600' : 'text-red-600'}`}>
                    {averageEfficiency}%
                  </span>
                </div>
              ...
              <div className="border-t pt-2">
                ...
                <div className="flex justify-between text-sm">
                  ...
                  <span className="text-blue-600">{shortestPath.length > 0 ? shortestPath.length - 1 : 0}</span>
                </div>
              ...
              <div className="border-t pt-2 text-sm">
                <div className="flex justify-between">
                  ...
                  <span className={
                    gameOver ? "text-red-600" : gameStarted ? "text-green-600" : "text-gray-500"
                  }>
          ...
          {/* Queue Action Panel */}
          <div className="bg-white rounded-lg shadow-md p-3 flex-1">
            <h4 className="text-sm font-bold text-gray-800 mb-3">Queue Action</h4>
            
            <div className="space-y-2">
              {/* Current Direction */}
              <div className="flex items-center justify-between text-xs">
                <span className="text-gray-600">Current:</span>
                <span className="text-lg font-bold text-orange-500">{getDirectionName(direction)}</span>
              </div>
              ...
              <div className="border-t pt-2">
                <div className="flex items-center justify-between text-xs mb-1">
                  ...
                  <span className="text-gray-500">{directionQueue.length}/2</span>
                </div>
                <div className="min-h-[40px] space-y-1">
                  {directionQueue.length === 0 ? (
                    <div className="text-gray-400 italic text-xs text-center py-2">Empty</div>
                  ) : (
                    directionQueue.map((dir, index) => (
                      <div key={index} className="flex items-center justify-between bg-blue-50 px-2 py-1 rounded text-xs">
                        ...
                        <span className="bg-blue-200 text-blue-800 px-1 rounded text-xs">#{index + 1}</span>
                      </div>
                    ))
                  )}
                </div>
              ...
              {/* Action History */}
              <div className="border-t pt-2">
                <h5 className="text-xs text-gray-600 mb-1">Action History (last 10):</h5>
                <div className="max-h-32 overflow-y-auto space-y-1">
                  {actionHistory.slice(-10).reverse().map((action: ActionHistoryItem, index) => {
                    const getActionColor = (type: string) => {
                      switch(type) {
                        case 'applied_immediate': return 'bg-green-100 text-green-800';
                        case 'applied_queue': return 'bg-blue-100 text-blue-800';
                        case 'queued': return 'bg-yellow-100 text-yellow-800';
                        case 'cancelled': return 'bg-red-100 text-red-800';
                        case 'game_over': return 'bg-purple-100 text-purple-800';
                        default: return 'bg-gray-100 text-gray-800';
                      }
                    };
                    
                    const getActionLabel = (type: string, reason?: string) => {
                      switch(type) {
                        case 'applied_immediate': return 'Applied ↗';
                        case 'applied_queue': return 'Applied ⚡';
                        case 'queued': return 'Queued 📝';
                        case 'cancelled': return reason === 'opposite' ? 'Blocked ⚠' : 'Ignored 🔄';
                        case 'game_over': return reason === 'wall' ? 'Hit Wall 💥' : 'Hit Self 💀';
                        default: return type;
                      }
                    };

                    return (
                      <div key={actionHistory.length - index} className="flex items-center justify-between text-xs">
                        <div className="flex items-center gap-2">
                          <span className="text-base">{getDirectionName(action.direction)}</span>
                          <span className={`px-1 rounded text-xs ${getActionColor(action.type)}`}>
                            {getActionLabel(action.type, action.reason)}
                          </span>
                        </div>
                        <span className="text-gray-400 text-xs">
                          {new Date(action.timestamp).toLocaleTimeString().split(' ')[0].split(':').slice(-2).join(':')}
                        </span>
                      </div>
                    );
                  })}
                  {actionHistory.length === 0 && (
                    <div className="text-gray-400 italic text-xs text-center py-2">No actions yet</div>
                  )}
                </div>
              </div>
              ...
              <div className="border-t pt-2 text-xs space-y-1">
                <div className="flex justify-between">
                  <span>Length:</span><span className="font-mono">{snake.length}</span>
                </div>
                <div className="flex justify-between">
                  <span>Status:</span>
                  <span className={
